...
use stroll_core::{
    models::{
        ...
        GeoLocation, Location, NetworkResponse, VoiceQuery,
    },
...
struct AppState {
    ...
    eventbrite_api_key: Option<String>,
...
async fn recommend(
    ...
    let live_activities = if let (Some(loc), Some(api_key)) = (&location, &state.google_places_api_key) {
        match fetch_google_places(&state.http_client, api_key, loc, &request.query).await {
            Ok(places) if !places.is_empty() => {
                println!("[PLACES] Fetched {} live venues from Google Places API", places.len());
                Some(places)
            }
            Ok(_) => {
            ...
            Err(e) => {
                eprintln!("[PLACES] Google Places API error: {}, falling back to local data", e.payload.message);
                None
            }
        }
    } else {
    ...
    // Build the response: use live data if available, otherwise fallback to StrollCore mock
    let mut response = if let Some(activities) = live_activities {
        let filters = activities.iter()
        ...
        AgentResponse {
            summary: format!("Found {} places near you matching '{}'", activities.len(), request.query),
            activities,
            filters,
            query: VoiceQuery {
    ...
    match maybe_enhance_summary(&state, provider, location, &request.query, &response.activities).await {
        Ok(Some(summary)) => {
...
async fn feed_handler(
    ...
    let live_recs = match (params.lat, params.lng, &state.google_places_api_key) {
        (Some(lat), Some(lng), Some(api_key)) => {
            ...
            let query = params.q.as_deref().unwrap_or("popular restaurants bars cafes things to do");
            match fetch_google_places(&state.http_client, api_key, &loc, query).await {
                Ok(places) if !places.is_empty() => {
                    println!("[FEED] Fetched {} live venues from Google Places", places.len());
                    Some(places)
                }
                Ok(_) => {
    ...
    if let Some(recs) = live_recs {
        ...
        let trending: Vec<String> = recs.iter()
        ...
        let response = state.core.get_personalized_feed().await.map_err(ApiError::from)?;
        Ok(Json(response))
    }
}
...
async fn trending_handler(
    ...
    let response = state.core.get_trending(query.category).await.map_err(ApiError::from)?;
    Ok(Json(response))
}

async fn saved_activities_handler(State(state): State<AppState>) -> Result<Json<Vec<Activity>>, ApiError> {
    let response = state.core.get_saved_activities().await.map_err(ApiError::from)?;
    Ok(Json(response))
}
...
async fn save_activity_handler(
    ...
    state.core.save_activity(request.activity_id).await.map_err(ApiError::from)?;
...
async fn follow_user_handler(
    ...
    state.core.follow_user(request.user_id).await.map_err(ApiError::from)?;
    Ok((StatusCode::OK, Json(json!({"ok": true}))).into_response())
}
...
async fn places_search_handler(
    ...
    let loc = GeoLocation { lat: request.lat, lng: request.lng };
...
async fn fetch_google_places(
    ...
    let parsed: Value = serde_json::from_str(&response_body).map_err(|e| {
        ApiError::upstream_error(format!("Google Places JSON invalid: {e}"))
    })?;
        ...
        .map(|(idx, place)| {
            ...
            Activity {
                ...
                location: Location {
                    ...
                    city: address.split(',').last().unwrap_or("").trim().to_string(),
                },
                rating,
                review_count,
                price_level,
...
fn guess_category_from_types(types: &[String]) -> ActivityCategory {
    for t in types {
        ...
        if lower.contains("restaurant") || lower.contains("food") || lower.contains("cafe")
            || lower.contains("bakery") || lower.contains("coffee")
        {
        ...
        if lower.contains("gym") || lower.contains("spa") || lower.contains("wellness")
            || lower.contains("yoga")
        {
            return ActivityCategory::Wellness;
        }
        if lower.contains("museum") || lower.contains("art") || lower.contains("theater")
        ...
        if lower.contains("amusement") || lower.contains("bowling") || lower.contains("cinema")
...
async fn maybe_enhance_summary(
    ...
    if let (Some(loc), Some(key)) = (&location, &state.openweathermap_api_key) {
        let url = format!("https://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&appid={}&units=metric", loc.lat, loc.lng, key);
    ...
    match provider {
        ...
        ProviderKind::OpenRouter => {
            ...
            let summary =
                generate_openrouter_summary(&state.http_client, config, query, activities, &context_str).await?;
            Ok(Some(summary))
        }
        ProviderKind::HuggingFace => {
            ...
            let summary =
                generate_huggingface_summary(&state.http_client, config, query, activities, &context_str).await?;
            Ok(Some(summary))
        }
        ProviderKind::Gemini => {
            ...
            let summary =
                generate_gemini_summary(&state.http_client, config, query, activities, &context_str).await?;
            Ok(Some(summary))
        }
    }
}
